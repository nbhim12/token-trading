...
import { PriceCell, MarketCapCell, VolumeCell, LiquidityCell } from "./PriceCell";
...
/**
 * Complete token row for table display
 * Combines all cell components into a single row
 */
export const TokenRow = memo(function TokenRow({
    ...
    return (
      <div
        className={cn(
          ...
          "border-b border-border-primary hover:bg-bg-hover/50 transition-colors cursor-pointer",
          className
        )}
        onClick={() => onViewDetails?.(token.id)}
      >
        <TokenNameCell token={token} size="sm" showAddress={false} className="flex-1 min-w-0" />
        <PriceCell price={token.price} priceChange={token.priceChange24h} size="sm" />
  ...
  return (
    <div
      className={cn(
        "group relative flex items-center gap-4 px-4 py-3",
        "border-b border-border-primary hover:bg-bg-hover/50 transition-colors",
        className
      )}
    >
      {/* Token Name - flex-1 to take remaining space */}
  ...
  return (
    <div
      className={cn(
        ...
        "hover:border-border-secondary transition-colors cursor-pointer",
        className
      )}
      onClick={() => onViewDetails?.(token.id)}
    >
