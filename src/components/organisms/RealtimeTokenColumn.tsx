...
import { RealtimeTokenRow, RealtimeTokenCard } from "@/components/molecules/RealtimeTokenRow";
...
import { useRecentlyUpdatedTokens } from "@/hooks/useRealtimeTokens";
import type { Token, TokenStatus } from "@/lib/types";

interface RealtimeTokenColumnProps {
  title: string;
  status: TokenStatus;
  tokens: Token[];
  isLoading?: boolean;
...
/**
 * Token column with real-time price updates
 * Uses RealtimeTokenRow for live price flash animations
 */
export const RealtimeTokenColumn = memo(function RealtimeTokenColumn({
  title,
  status,
  tokens,
  isLoading = false,
  ...
  favoriteIds = new Set(),
  variant = "table",
}: RealtimeTokenColumnProps) {
  const { sortedData, sortConfig, handleSort } = useTableSort<Token>(tokens, {
    key: "createdAt",
    direction: "desc",
  });

  const recentlyUpdatedIds = useRecentlyUpdatedTokens();
  const currentSortKey = sortConfig?.key ?? null;
  const currentDirection = sortConfig?.direction ?? null;

  // Count of recently updated tokens in this column
  const updateCount = sortedData.filter((t) => recentlyUpdatedIds.has(t.id)).length;

  if (variant === "card") {
    return (
      <div className={cn("flex flex-col h-full", className)}>
        {/* Header */}
        <RealtimeColumnHeader
          title={title}
          status={status}
          count={tokens.length}
...
/**
 * Column header with real-time update indicator
 */
const RealtimeColumnHeader = memo(function RealtimeColumnHeader({
  title,
  status,
  count,
  updateCount,
  ...
  return (
    <div className="flex items-center justify-between px-4 py-3 border-b border-border-primary bg-bg-secondary">
      <div className="flex items-center gap-2">
        ...
        {updateCount > 0 && (
          <span className="flex items-center gap-1 text-[10px] text-success animate-pulse">
...
/**
 * Compact realtime token row for column view
 */
const CompactRealtimeRow = memo(function CompactRealtimeRow({
  token,
  onBuy,
  onFavorite,
  onViewDetails,
